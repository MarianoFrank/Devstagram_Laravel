    ...
    <header class="flex justify-between items-center py-10 mx-auto max-w-screen-xl w-[90%]">
        ...
        <nav class="flex gap-5  items-center ">
            ...
            @auth
                ...
                <a class="rounded px-2 py-1 hover:bg-slate-100 border border-slate-200" href="{{ route('post.create') }}"><i
                        class="bi bi-camera"></i> Crear</a>

                <a href="{{ route("post.index",Auth::user()->username) }}" class="hover:text-primario-300">Hola,
                    {{ auth()->user()->name }}</a>
